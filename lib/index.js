...
export default {
  ...
  consumeIndie(registerIndie) {
    ...
    this.subscriptions.add(atom.workspace.observeTextEditors((textEditor) => {
      ...
      const replay = () => this.npmWorker.check(textEditor, {skipRegistryCall: true})
        .then(setMessages)
        .catch();

      this.npmWorker.check(textEditor, {replay})
        .then(setMessages)
        .catch();

      const subscription = textEditor.onDidStopChanging(() => this.npmWorker.check(textEditor, {replay})
        .then(setMessages)
        .catch()
      );
