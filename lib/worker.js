...
import resolveDep from "resolve";
...
export class NpmWorker {
  ...
  getInstalledVersion(dependency, directory) {
    return new Promise(resolve => {
      resolveDep(dependency.name, { basedir: directory }, (err, res, pkg) => {
        if (err || !pkg || !pkg.version) {
          return resolve({ installedVersion: "0.0.0" });
        }
        resolve({ installedVersion: pkg.version });
      });
    });
  }
  ...
  check(textEditor, options) {
    ...
    const directory = path.dirname(textEditor.getPath());
    let dependencies = [];
    try {
      ...
      .map(dependency =>
        (this.checkInstalled
          ? Promise.all([
              ...
              this.getInstalledVersion(dependency, directory)
