...
export class NpmWorker {
  ...
  isRefreshNeeded(pck) {
    ...
    return (
      !pck.infoDate ||
      pck.infoDate >
        new Date(now.getTime() - this.cacheRefreshFrequency * 60000)
    );
  }
   ...
   * @return {Promise} Dependency with its available versions list
   */
  getAvailableVersions(dependency, skipRegistryCall) {
    const refresh = pck => {
      if (this.isRefreshNeeded(pck) && !skipRegistryCall) {
    ...
    if (pck) {
      ...
      return Promise.resolve({
        ...
        availableVersions: this.extractValuableVersions(
          this.packages[dependency.name].versions
        )
      });
    } else {
      ...
      return cacheP.then(store => {
        const pck = store && store[dependency.name];
        if (!pck && skipRegistryCall) { return undefined; }
        return ((pck) ? refresh(pck) : this.updatePackages(dependency))
          .then((pkg) => {
            storage.set(this.CACHE_NAME, Object.assign(store || {}, this.packages), () => {});
            return pkg;
          })
          .then((pkg) => ({
            name: dependency.name,
  ...
  check(textEditor, options) {
    const opts = Object.assign({skipRegistryCall: false}, options);
    ...
    try {
      ...
      const reports = dependencies
        ...
        .map(dependency =>
          (this.checkInstalled
            ? Promise.all([
                this.getAvailableVersions(dependency, opts.skipRegistryCall),
      ...
      return Promise.all(reports).then(reports => this.linter.filter(reports));
    } catch (err) {
      return [];
    }
  }
  ...
  suggestion(textEditor, bufferPosition) {
    ...
    /* eslint-disable no-console */
    this.updatePackages(dependency).catch(dependency =>
      console.error(`Unable to update ${dependency.name}`)
    );
    /* eslint-enable no-console */
