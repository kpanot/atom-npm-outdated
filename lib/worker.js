...
export class NpmWorker {
  ...
  fecthDependency(filepath, dependency) {
    return this.getRegistry(path.dirname(filepath))
      ...
      .then((registry) => fetch(`${_.trim(registry, '/')}/${dependency.name}`))
  }
  ...
  getAvailableVersions(filepath, dependency) {
    return this.fecthDependency(filepath, dependency)
      ...
      .then((res) => ({
          ...
          availableVersions: _.keys(res.versions).filter((version) => this.beta || !semver.prerelease(version))
        })
      );
  }
  ...
  toLinterReport(dependency, content, textEditor) {
    ...
    let type = 'Error';
    let text = `The package <b>${dependency.name}</b> should be upgraded from <b>${dependency.version}</b> to <b>${latest}</b>`;

    if (semver.satisfies(latest, dependency.version)) {
        type = 'Info';
        text = `The package <b>${dependency.name}</b> is up-to-date (latest version: ${latest})`;
    } else if (!!semver.prerelease(latest)) {
      type = 'Warning';
      text = `The package <b>${dependency.name}</b> can be upgraded from <b>${dependency.version}</b> to <b>${latest}</b>`;
    }
