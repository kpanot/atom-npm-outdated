...
export class NpmWorker {
   ...
   * @return {Promise} Dependency with its available versions list
   */
  getAvailableVersions(dependency, options) {
    const opts = Object.assign({skipRegistryCall: false, replay: () => {}}, options);
    const refresh = pck => {
      if (this.isRefreshNeeded(pck) && !opts.skipRegistryCall) {
    ...
    if (pck) {
      refresh(pck)
        .then((newPck) => {
          if (!opts.skipRegistryCall && JSON.stringify(newPck) !== JSON.stringify(pck)) {
            opts.replay();
          }
        })
        .catch();
      ...
      return cacheP.then(store => {
        ...
        if (!pck && opts.skipRegistryCall) { return undefined; }
    ...
    try {
      ...
      const reports = dependencies
        ...
        .map(dependency =>
          (this.checkInstalled
            ? Promise.all([
                this.getAvailableVersions(dependency, opts),
