 ...
 * @type {String}
 */
const DEFAULT_REGISTRY = {default: 'https://registry.npmjs.org', scoped: {}};
...
export class NpmWorker {
   ...
   * @param {String} npmrcFile path to the .npmrc file to read
   * @return {Promise} Registries list
   */
  getRegistriesFromFile(npmrcFile) {
    return new Promise((resolve, reject) =>
      fs.readFile(npmrcFile, (err, data) => {
        if (err)  {
          return reject(err);
        }
        const registries = Object.assign({}, DEFAULT_REGISTRY);
        const registryLine = /^registry=(.*)$/gm.exec(data);
        const scopedRegistryLines = /^(@[^:]+):registry=(.*)$/gm.exec(data);

        if (registryLine && registryLine.length > 0) {
          registries.default = registryLine[1];
        }

        if (scopedRegistryLines) {
          for(let i = 0; i < scopedRegistryLines.length;i += 3) {
            registries.scoped[scopedRegistryLines[i + 1]] = scopedRegistryLines[i + 2];
          }
        }
  ...
  /**
   * Get the registry url
   * @param {String} filepath path to the file opened
   * @return {Promise} URL to the npm registry
   */
  getRegistries(filepath) {
    const file = path.join(atom.project.relativizePath(filepath)[0], this.npmrc);

    const getRegistries = this.getRegistriesFromFile(file)
      .then((registries) => this.registries = registries);
   ...
   */
  fecthDependency(dependency, filepath) {
    return this.getRegistries(filepath)
      .catch(() => DEFAULT_REGISTRY)
      .then((registries) => {
        const registry = this.getPackageRegistry(registries, dependency);
        return fetch(`${registry.replace(/\/+$/, '')}/${dependency.name.replace('/', '%2f')}`);
      })
   ...
   */
  getAvailableVersions(dependency, filepath) {
    const registryVersions = this.fecthDependency(dependency, filepath)
      .then((res) => res.json())
      .then((res) => {
  ...
  check(textEditor) {
    ...
    try {
      ...
      const reports = dependencies.map((dependency) =>
        (this.checkInstalled ?
          Promise.all([this.getAvailableVersions(dependency, filepath), this.getInstalledVersion(dependency)])
            .then((dependencyPartials) => Object.assign({}, ...dependencyPartials)) :
          this.getAvailableVersions(dependency, filepath)
        )
          .then((dependency) => this.linter.reportDependency(dependency, content, textEditor))
          .catch(() => this.getRegistries(filepath)
            .catch(() => DEFAULT_REGISTRY)
            .then((registries) => this.linter.reportUnfound(dependency, content, textEditor, this.getPackageRegistry(registries, dependency))))
      );
  ...
  suggestion (textEditor, bufferPosition) {
    ...
    return this.getAvailableVersions(dependency, filepath)
