...
export class NpmWorker {
  ...
  toLinterReport(dependency, content, textEditor) {
    ...
    let type = 'Info';
    let text = `The package <b>${dependency.name}</b> is up-to-date (latest version: ${latest})`;
    ...
    if (!semver.satisfies(latest, dependency.version)) {
      if (semver.satisfies(latestStable, dependency.version)) {
        ...
        type = 'Error';
        text = `The package <b>${dependency.name}</b> should be upgraded from <b>${dependency.version}</b> to <b>${latest}</b>`;
