...
export class NpmWorker {
  ...
  /**
   * Get the list of dependency fields in the package.json
   * @param  {Object} packageJson Package.json object
   * @return {String[]}             List of fields to handle
   */
  getListOfDependencyFields(packageJson) {
   ...
   * @return {Object[]} List of package dependencies
   */
  decodeDependencies(packageJson) {
    const dependencyFields = this.getListOfDependencyFields(packageJson);
    const dependenciesOnly = {};
    dependencyFields.forEach((field) => dependenciesOnly[field] = packageJson[field]);
    return dependenciesOnly;
  }

  /**
   * Get the full list of dependency names
   * @param  {Object} packageJson Package.json content
   * @return {String[]}             List of dependencies names
   */
  getFlatDependenciesList(packageJson) {
    const dependenciesLists = this.decodeDependencies(packageJson);

    return [...Object.keys(dependenciesLists)
      .map((dependenciesKey) => Object.keys(dependenciesLists[dependenciesKey]))
    ];
  }
  ...
  parseJson(content) {
    ...
    return [...Object.keys(dependenciesLists)
      .map((dependenciesKey) => {
        const dependencies = dependenciesLists[dependenciesKey];
        return Object.keys(dependenciesLists[dependenciesKey])
          .map((name) => ({name: name, version: dependencies[name], origin: dependenciesKey}));
      })
    ]
  }
  ...
  check(textEditor) {
    ...
    try {
      const dependenciesList = this.parseJson(content);
      const reportsList = dependenciesList.map((dependencies) =>
      ...
      return Promise.all(reportsList.reduce((acc, reports) => [...acc, ...reports], []))
  ...
  isDependency (content, dependencyName) {
    const packageJson = JSON.parse(content);
    const combineDependencies = this.getFlatDependenciesList(packageJson);
    return !!combineDependencies.find((dep) => dep === dependencyName);
  }
